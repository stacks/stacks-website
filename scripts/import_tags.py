...
def insert_tag(tag, value):
  try:
    if tag_exists(tag):
      ...
      connection.execute(query, (value[0], value[1], value[2], value[3], value[4], value[5], value[6], value[7], tag))
    else:
      query = 'INSERT INTO tags (tag, label, file, chapter_page, book_page, book_id, name, type, position) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
      connection.execute(query, (tag, value[0], value[1], value[2], value[3], value[4], value[5], value[6], value[7]))
